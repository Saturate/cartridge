...
use crate::agent::{
    ...
    spawn::{spawn_agent, validate_env, SpawnRequest},
...
pub async fn create_agent(
    ...
    let provider = Provider::from_str(&req.provider).ok_or_else(|| {
        err(
    ...
    if provider == Provider::Custom && req.command.is_empty() {
        return Err(err(
            StatusCode::BAD_REQUEST,
            "invalid_command",
            "command is required for custom provider",
        ));
    }
...
pub async fn send_input(
    ...
    if let Some(tx) = &a.pty_cmd_tx {
        tx.send(PtyCommand::Input(data)).await.map_err(|_| {
            err(
...
pub async fn resize_agent(
    ...
    if a.headless {
        return Err(err(
            ...
            "headless_no_resize",
            "Headless agents do not have a PTY to resize",
        ));
    }
